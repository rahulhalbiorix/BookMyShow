  ...
  <div class="max-w-7xl mx-auto p-6 text-white min-h-screen">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
              <div>
                ...
                <Dropdown
                  ...
                  :disabled="isEditMode"
                />
                <p v-if="errors.theaterId" class="text-red-400 text-xs mt-1">
                  {{ errors.theaterId }}
                </p>
              </div>
              ...
              <div>
                ...
                <Dropdown
                  ...
                  :disabled="isEditMode"
                />
                <p v-if="errors.screenId" class="text-red-400 text-xs mt-1">
                  {{ errors.screenId }}
                </p>
              </div>
              ...
              <div class="flex gap-4">
                <div class="w-1/2">
                  ...
                  <Calendar
                    ...
                    :disabled="isEditMode"
                  />
                  <p v-if="errors.showStartDate" class="text-red-400 text-xs mt-1">
                    {{ errors.showStartDate }}
                  </p>
                </div>
                <div class="w-1/2">
                  ...
                  <Calendar
                    ...
                    :disabled="isEditMode"
                  />
                  <p v-if="errors.showEndDate" class="text-red-400 text-xs mt-1">
                    {{ errors.showEndDate }}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <Calendar
                  ...
                  :disabled="isEditMode"
                />
                <p v-if="errors.startTime" class="text-red-400 text-xs mt-1">
                  {{ errors.startTime }}
                </p>
              </div>
              ...
              <div>
                ...
                <InputNumber
                  ...
                  class="w-full"
                />
                <p v-if="errors.ticketPrice" class="text-red-400 text-xs mt-1">
                  {{ errors.ticketPrice }}
                </p>
              </div>
              ...
              <div class="flex justify-end gap-3 pt-4">
                ...
                <Button
                  :label="loading ? 'Saving...' : SubmitBtnLable"
                  icon="pi pi-save"
                  type="submit"
                  severity="danger"
                  :disabled="loading"
      ...
      <div class="lg:col-span-2">
        ...
        <p v-if="errors.movieId && !isEditMode" class="text-red-400 text-xs mt-2">
          {{ errors.movieId }}
        </p>
...
const SubmitBtnLable = computed(() => (editShowId.value ? 'Edit Show' : 'Add Show'))
...
watch(
  ...
  (newTheaterId) => {
    if (newTheaterId) handleFetchscreenList(newTheaterId)
  },
)
...
const validateForm = () => {
  errors.value = {}
  if (!form.value.theaterId) errors.value.theaterId = 'Theater is required'
  if (!form.value.screenId) errors.value.screenId = 'Screen is required'
  if (!form.value.showStartDate) {
    errors.value.showStartDate = 'Show start date is required'
  }
  if (!form.value.showEndDate) {
    errors.value.showEndDate = 'Show end date is required'
  } else if (form.value.showEndDate < form.value.showStartDate) {
    errors.value.showEndDate = 'End date cannot be before start date'
  }
  if (!form.value.startTime) errors.value.startTime = 'Start time is required'
  if (!form.value.ticketPrice || form.value.ticketPrice <= 0) {
    errors.value.ticketPrice = 'Ticket price must be greater than 0'
  }
  if (!form.value.movieId && !isEditMode.value) {
    errors.value.movieId = 'Please select a movie'
  }
...
onMounted(() => {
  ...
  if (editShowId.value) fetchShowDetails(editShowId.value)
})
</script>
